...
def add_task_options(parser, only_MNIST=False, single_task=False, compare_code="none", **kwargs):
    ...
    if single_task:
        task_choices = ['CIFAR10', 'CIFAR100', 'MNIST', 'MNIST28','ASL','fruits360','chars74K','gtsrb']
        task_default = 'CIFAR10'
    else:
        MNIST_tasks = ['splitMNIST', 'permMNIST', 'fmnist']
        image_tasks = ['CIFAR100','ASL','fruits360','chars74K','gtsrb']
    ...
    if not only_MNIST:
        ...
        task_params.add_argument('--no-norm', action='store_false', dest='normalize',
                                 help="don't normalize images (only for 'CIFAR100','ASL','fruits360','chars74K' and 'gtsrb')")
...
def set_defaults(args, only_MNIST=False, single_task=False, generative=True, compare_code='None', **kwargs):
    ...
    args.normalize = args.normalize if args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"] else False
    args.augment = args.augment if args.experiment in ('CIFAR10', 'CIFAR100') else False
    if hasattr(args, "depth"):
        args.depth = (5 if args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"] else 0) if args.depth is None else args.depth
    if hasattr(args, "recon_loss"):
        args.recon_loss = (
            "MSE" if args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"] else "BCE"
    ...
    if not single_task:
        args.tasks= (
            5 if args.experiment in ['splitMNIST', 'fmnist'] else (10 if args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"] else 100)
        ) if args.tasks is None else args.tasks
        args.iters = (5000 if args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"] else 2000) if args.iters is None else args.iters
        args.lr = (0.001 if args.experiment in ['splitMNIST', 'fmnist'] else 0.0001) if args.lr is None else args.lr
        args.batch = (128 if args.experiment in ['splitMNIST', 'fmnist'] else 256) if args.batch is None else args.batch
    ...
    if not single_task and not compare_code in ('hyper', 'bir'):
        ...
        elif args.experiment in ["CIFAR100", "ASL", "fruits360", "chars74k", "gtsrb"]:
