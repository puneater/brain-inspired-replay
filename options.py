...
def set_defaults(args, only_MNIST=False, single_task=False, generative=True, compare_code='None', **kwargs):
    ...
    if not single_task:
        args.tasks= (
            5 if args.experiment in ['splitMNIST', 'fmnist'] else (10 if args.experiment=="CIFAR100" else 100)
    ...
    if not single_task and not compare_code in ('hyper', 'bir'):
        if args.experiment in ['splitMNIST', 'fmnist','ASL','fruits360','chars74K','gtsrb']:
